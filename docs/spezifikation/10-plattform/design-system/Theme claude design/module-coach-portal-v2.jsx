    ...
    <div>
      ...
      {sel.length > 0 && (
        <div style={{display: "flex", alignItems: "center", gap: 9, padding: "9px 13px", marginBottom: 12,
          ...
          <button className="btn btn-sm"><Icon name="message" className="ic ic-sm"/>Message all</button>
          <button className="btn btn-sm"><Icon name="workord" className="ic ic-sm"/>Assign plan</button>
          <button className="btn btn-sm"><Icon name="calendar" className="ic ic-sm"/>Schedule check-in</button>
          <button className="btn btn-sm"><Icon name="download" className="ic ic-sm"/>Export</button>
            ...
            <tr>
              <th style={{width: 26}}>
                <input type="checkbox"
                  ...
                  onChange={() => setSel(sel.length === rows.length ? [] : rows.map(a => a.id))}
                ...
                <tr key={a.id} className="clickable" style={{cursor: "pointer"}}
                  ...
                  <td onClick={e => e.stopPropagation()}>
                    <input type="checkbox" checked={sel.includes(a.id)}
                      onChange={() => setSel(s => s.includes(a.id) ? s.filter(x => x !== a.id) : [...s, a.id])}
                      style={{accentColor: "var(--acc-coach)", cursor: "pointer"}}/>
                  </td>
