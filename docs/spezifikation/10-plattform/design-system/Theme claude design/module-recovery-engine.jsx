// Recovery · spec-compliant engine — 3 score modes, 18-muscle map, HRV PPG,
// overtraining signals, protocols, pending actions

// ── 18 body-map muscle groups (F3) ────────────────────────
...
// Training-module muscle name → body-map slug
...
// SVG geometry per slug — {view, d} paths on a 100×120 outline
// Anatomical silhouette (viewBox 0 0 100 126), arms abducted so the arm muscles
// have real geometry to sit on. Head+torso+legs is one subpath, each arm is its own;
// nonzero fill unions them at the shoulder.
const SILHOUETTE_PATH = [
  ...
  // left arm — widest at the deltoid, tapering continuously to the wrist
  ...
  // right arm — same winding: inner edge down, outer edge up
...
// Muscle shapes drawn onto that figure. Arm muscles sit inside the arm subpaths,
// torso muscles inside the trunk. Clipping is a safety net, not the mechanism.
const MUSCLE_PATHS = {
  // ── FRONT ──────────────────────────────────────────────
...
// ── Muscle recovery formula (F3 / SPEC_05) ────────────────
const lerp = (a, b, t) => a + (b - a) * Math.max(0, Math.min(1, t));

function baseRecoveryCurve(h) {
...
const volumeMod   = s => s <= 6 ? 1.10 : s <= 12 ? 1.00 : s <= 18 ? 0.85 : s <= 24 ? 0.70 : 0.50;
const sleepMod    = q => q >= 8.5 ? 1.15 : q >= 7 ? 1.00 : q >= 5 ? 0.85 : 0.65;
const nutritionMod = (p, c) => (p >= 0.9 && c >= 0.95) ? 1.1 : p >= 0.8 ? 1.0 : p >= 0.6 ? 0.8 : 0.6;
const sorenessMod = v => v === 0 ? 1.1 : v === 1 ? 1.0 : v === 2 ? 0.75 : 0.50;

function calcMuscleRecovery({ hours, sets, sleepQuality, proteinPct, caloriePct, soreness }) {
  const base = baseRecoveryCurve(hours);
  const vm = volumeMod(sets), sm = sleepMod(sleepQuality);
  const nm = nutritionMod(proteinPct, caloriePct), som = sorenessMod(soreness);
  return {
...
// ── Per-muscle training state (from Training module) ──────
const MUSCLE_STATE = {
...
const CHECKIN = {
  ...
  mood: "good",
  soreness: Object.fromEntries(Object.entries(MUSCLE_STATE).map(([k,v]) => [k, v.soreness])),
  stress_level: 3,
  alcohol_units: 0,
  caffeine_mg: 280,
...
const MOOD_MULTIPLIER = { motivated:1.0, good:0.8, neutral:0.6, tired:0.3, sick:0.1 };
const MOOD_META = [
...
const TODAY_MODALITIES = MODALITY_LOG.filter(m => m.date === "2026-08-15");

function calcModalityBonus(mods) {
  const raw = mods.reduce((s, m) => s + (MODALITY_BONUS[m.type] ?? 0), 0);
  return { raw: Math.round(raw * 10) / 10, capped: Math.min(raw, MAX_DAILY_BONUS), wasCapped: raw > MAX_DAILY_BONUS };
}

// ── ACWR / training load (F2) ─────────────────────────────
const ACWR_DATA = { acute_7d: 2142, chronic_28d: 1980, acwr: 1.08 };
function calcTrainingLoadScore(acwr) {
...
// ── HRV (F4) ──────────────────────────────────────────────
const HRV_BASELINE = { avg_rmssd: 58.4, stddev_rmssd: 6.2, window_days: 30, samples: 28 };
function calcHRVScore(rmssd, base = HRV_BASELINE) {
  const z = (rmssd - base.avg_rmssd) / base.stddev_rmssd;
  return { score: Math.max(0, Math.min(100, Math.round(70 + z * 15))), z: Math.round(z * 100) / 100 };
}
const HRV_LOG = [
...
function calcSleepScore(data, checkin) {
  if (data) {
...
const readinessFor = s => READINESS_LEVELS.find(l => s >= l.min) || READINESS_LEVELS[READINESS_LEVELS.length-1];

// ── Overtraining · 8 signals (F7) ─────────────────────────
const OT_DATA = {
...
function evaluateOvertraining(data = OT_DATA) {
  const results = OVERTRAINING_SIGNALS.map(s => ({ ...s, fired: s.check(data), detailText: s.detail(data) }));
...
const ACTIVE_PROTOCOL = { id:"active_recovery_week", day:3, of:7, started:"2026-08-13" };

// ── Pending actions (F10) ─────────────────────────────────
function recoveryPendingActions() {
  const out = [];
  const hour = 14;
...
Object.assign(window, {
  SILHOUETTE_PATH,
  MUSCLE_GROUPS_BODYMAP, MUSCLE_LABEL, MUSCLE_SLUG_MAP, MUSCLE_PATHS, MUSCLE_STATE,
  baseRecoveryCurve, calcMuscleRecovery, volumeMod, sleepMod, nutritionMod, sorenessMod,
  CHECKIN, MOOD_MULTIPLIER, MOOD_META,
  MODALITY_BONUS, MAX_DAILY_BONUS, MODALITY_META, MODALITY_LOG, TODAY_MODALITIES, calcModalityBonus,
  ACWR_DATA, calcTrainingLoadScore,
  HRV_BASELINE, calcHRVScore, HRV_LOG,
  SLEEP_DATA, calcSleepScore,
  calcRecoveryScore, avgSoreness, READINESS_LEVELS, readinessFor,
  OT_DATA, OVERTRAINING_SIGNALS, evaluateOvertraining,
  RECOVERY_PROTOCOLS, ACTIVE_PROTOCOL, recoveryPendingActions,
});
