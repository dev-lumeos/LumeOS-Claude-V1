...
const App = () => {
  ...
  useEffect(() => {
    const h = (e) => setActive(e.detail || "dashboard");
    window.addEventListener("gonav", h);
    return () => window.removeEventListener("gonav", h);
  }, []);
  ...
  const moduleEl = (() => {
    switch (active) {
      ...
      case "portal": return window.CoachPortalPlatform ? <window.CoachPortalPlatform /> : null;
  ...
  // PORTAL TAKEOVER — coach.lumeos.app is its own platform, not a LumeOS module
  if (active === "portal") {
    return (
      <div style={{ height: "100vh", width: "100vw", overflow: "hidden", background: "var(--bg)" }}>
        {window.CoachPortalPlatform
          ? <window.CoachPortalPlatform onExit={() => setActive("dashboard")} theme={t.theme} setTheme={v => setTweak("theme", v)} />
          : null}
  ...
  return (
    ...
    >
      ...
      <main className="main">
        ...
        />
        <div className="content" data-platform={active === "portal" ? "true" : undefined}>{moduleEl}</div>
      </main>
      {t.rightPanel && (
        <ContextPanel
          moduleId={active}
