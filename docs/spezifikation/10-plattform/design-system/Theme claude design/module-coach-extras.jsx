...
const CoachPortalSmartAlerts = () => {
  ...
  return (
    <div>
      ...
      {alertSel.length > 0 && (
        <div style={{display: "flex", alignItems: "center", gap: 9, padding: "9px 13px", marginBottom: 12,
          background: "color-mix(in srgb, var(--acc-coach) 8%, var(--surface))",
          border: "1px solid color-mix(in srgb, var(--acc-coach) 30%, var(--border))", borderRadius: 7}}>
          <span className="num" style={{fontSize: 11.5, fontWeight: 600}}>{alertSel.length} selected</span>
          <div className="spacer"/>
          <button className="btn btn-sm">Acknowledge all</button>
          <button className="btn btn-sm">Resolve all</button>
          <button className="btn btn-sm">Snooze 24 h</button>
          <button className="btn btn-sm btn-ghost">Dismiss all</button>
          <button className="btn btn-sm btn-ghost" onClick={() => setAlertSel([])}>Clear</button>
        </div>
      )}
      <div style={{display: "flex", alignItems: "center", gap: 8, marginBottom: 9}}>
        <input type="checkbox"
          checked={alertSel.length > 0 && alertSel.length === filtered.length}
          onChange={() => setAlertSel(alertSel.length === filtered.length ? [] : filtered.map(a => a.id))}
          style={{accentColor: "var(--acc-coach)", cursor: "pointer"}}/>
        <span className="dim" style={{fontSize: 11}}>Select all {filtered.length}</span>
      </div>
      <div className="col-gap" style={{gap: 8}}>
        {filtered.map(a => <SmartAlertCard key={a.id} a={a} selected={alertSel.includes(a.id)} onSelect={() => setAlertSel(s => s.includes(a.id) ? s.filter(x => x !== a.id) : [...s, a.id])}/>)}
      </div>
    </div>
  );
};

const SmartAlertCard = ({ a, selected, onSelect }) => {
        ...
        <div style={{display: "flex", alignItems: "center", gap: 8, marginBottom: 6, flexWrap: "wrap"}}>
          {onSelect && (
            <input type="checkbox" checked={!!selected}
              ...
              style={{accentColor: "var(--acc-coach)", cursor: "pointer", marginRight: 2}}/>
          )}
