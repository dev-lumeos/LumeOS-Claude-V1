// Medical · spec-compliant rebuild — Dual-Range, 6 flags, 5 system scores,
// OCR import, symptoms, alerts, correlation engine, doctor export, privacy tiers

// ── 6-flag algorithm (SPEC_09 §2) ─────────────────────────
const FLAG_SCORE = { optimal: 100, normal: 75, low: 40, high: 40, critical_low: 10, critical_high: 10 };
const FLAG_META = {
...
function calcBiomarkerFlag(value, b, gender = "male") {
  const g = b.gender_ranges?.[gender] || {};
  const optMin = g.optimal_min ?? b.optimal_min, optMax = g.optimal_max ?? b.optimal_max;
  const labMin = g.lab_min ?? b.lab_min, labMax = g.lab_max ?? b.lab_max;
...
// ── Biomarker catalog · 9 categories, LOINC, dual ranges ──
const BIOMARKERS = [
  // CBC
...
const SYSTEM_WEIGHTS = { cardiovascular:0.25, metabolic:0.25, hormonal:0.20, liver:0.15, kidney:0.15 };
const SYSTEM_META = {
...
function calcSystemScore(system) {
...
function calcOverallHealthScore() {
  ...
  return { score, status: score>=85?"optimal":score>=70?"good":score>=55?"fair":"poor", data_completeness: totalW };
}

// ── Trend via linear regression (SPEC_09 §4) ──────────────
function calcBiomarkerTrend(hist) {
  const n = hist.length;
  if (n < 3) return { direction:"insufficient_data", strength:"negligible", slope:0, change_pct:0, projected:null, n };
  const xs = hist.map((_,i) => i), ys = hist;
  const meanX = xs.reduce((s,x)=>s+x,0)/n, meanY = ys.reduce((s,y)=>s+y,0)/n;
  const slope = xs.reduce((s,x,i)=>s+(x-meanX)*(ys[i]-meanY),0) / Math.max(xs.reduce((s,x)=>s+(x-meanX)**2,0),1);
...
// ── Medications v2 with monitoring (F6) ────────────────────
const MEDICATIONS_V2 = [
...
// ── Supplement effectiveness (F7) ──────────────────────────
const SUPPLEMENT_BIOMARKER_MAP = [
...
function calcSupplementEffectiveness(entry) {
  const b = BIOMARKERS.find(x => x.id === entry.biomarker);
  if (!b) return null;
  // Simplified: first 2 hist points = before, last = after
  const before = b.hist[1], latest = b.hist[b.hist.length - 1];
  ...
  return {
    ...
    status: becameOptimal ? "effective" : improving && Math.abs(changePct) >= 10 ? "partial" : Math.abs(changePct) < 5 ? "no_change" : "inconclusive",
  };
}

// ── Correlation engine (F8) ────────────────────────────────
const CORRELATIONS = [
...
Object.assign(window, {
  BIOMARKERS, BIOMARKER_CATEGORIES, FLAG_META, FLAG_SCORE,
  SYSTEM_MARKERS, SYSTEM_WEIGHTS, SYSTEM_META,
  calcBiomarkerFlag, calcSystemScore, calcOverallHealthScore, calcBiomarkerTrend,
  generateAlerts, SYMPTOMS, SYMPTOM_BIOMARKER_MAP, MEDICATIONS_V2,
  SUPPLEMENT_BIOMARKER_MAP, calcSupplementEffectiveness, CORRELATIONS,
  LAB_REPORTS, OCR_EXTRACTED, UNIT_CONVERSIONS,
});
