...
**Base URL:** `http://nutrition:5100`
**Auth:** JWT via `Authorization: Bearer <token>` (globalAuthMiddleware)
**User-ID:** aus JWT extrahiert → `userId` in allen Handlers
**Schema:** `search_path = nutrition`
**Format:** Alle Responses: `{ ok: boolean, data?: T, error?: string }`
...
| `category_id` | uuid | — | Filter auf Kategorie (inkl. Sub-Kategorien) |
| `tag` | string | — | Filter auf Tag-Code (z.B. 'high_protein') |
| `limit` | integer | 20 | Max Ergebnisse |
| `offset` | integer | 0 | Paginierung |
| `sort` | string | 'relevance' | relevance \| protein_desc \| kcal_asc \| name_asc |
| `include_custom` | boolean | true | Custom Foods einschliessen |
...
3. Trigger aktualisiert Recipe.total_*
...
Rezept updaten (name, servings, instructions, tags).
Items separat via Item-Endpoints.
...
**Body:** `FormData` mit `image` Feld (JPEG/PNG)
oder `{ "image_base64": "...", "mime_type": "image/jpeg" }`
