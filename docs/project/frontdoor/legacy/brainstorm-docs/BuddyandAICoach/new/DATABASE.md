# Buddy / AI Coach Module — Database Schema

## Schema & Übersicht

Alle Buddy-Tabellen im Schema `buddy`. Events sind append-only.
...
CREATE TABLE buddy.user_coach_profile (
  ...
  coach_personality  TEXT DEFAULT 'motivator'
    CHECK (coach_personality IN ('scientist','motivator','drill_sergeant','best_friend','zen_master')),
  ...
  feature_tier       TEXT DEFAULT 'free'
    CHECK (feature_tier IN ('free','plus','pro','elite')),
...
CREATE TABLE buddy.user_milestones (
  ...
  user_id      UUID NOT NULL,
  type         TEXT NOT NULL
    CHECK (type IN ('pr','streak','body','habit','social','custom')),
  title        TEXT NOT NULL,
  description  TEXT,
...
CREATE TABLE buddy.buddy_state (
  ...
  -- State Domains (JSONB für Flexibilität)
  ...
  -- Behavioral Signature (ab 8 Wochen)
  behavioral_signature JSONB DEFAULT '{}',

  -- Context Vector (für Intervention Engine)
...
CREATE TABLE buddy.buddy_rules (
  id                      UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id                 UUID,  -- NULL = globale System-Regel
...
CREATE INDEX idx_brules_category    ON buddy.buddy_rules(category, is_active);
```
...
CREATE TABLE buddy.intervention_log (
  ...
  intervention_type    TEXT NOT NULL
    CHECK (intervention_type IN ('confrontation','encouragement','adjustment','redirect','silence')),
  tone_variant         TEXT
...
CREATE TABLE buddy.coach_messages (
  id                UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  conversation_id   UUID NOT NULL REFERENCES buddy.coach_conversations(id) ON DELETE CASCADE,
  ...
  user_feedback     TEXT CHECK (user_feedback IN ('helpful','not_helpful','irrelevant',NULL)),
...
CREATE TABLE buddy.knowledge_base (
  ...
  summary          TEXT,
  category         TEXT
    CHECK (category IN ('nutrition','training','recovery','supplements','medical','psychology')),
  tags             TEXT[] DEFAULT '{}',
  content_type     TEXT,
  source           TEXT,
  evidence_level   TEXT CHECK (evidence_level IN ('A+','A','B','C','D',NULL)),
...
CREATE TABLE buddy.coach_journey (
  ...
  -- [{id, time: "07:00", modules: [...], persona, push: bool, active_days: [...], enabled: bool}]
...
## 15. buddy.coach_alerts (Proaktiver Wächter)
...
CREATE INDEX idx_calerts_user_active ON buddy.coach_alerts(user_id, dismissed, created_at DESC);
```

---

## 16. buddy.coach_profiles (Präferenzen + Konfiguration)
