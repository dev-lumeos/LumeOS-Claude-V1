...
- Governed operator runs now expose explicit orchestration mode selection: `auto`, `codex_bootstrap`, and `spark1_orchestrated`. Reports include requested mode, actual mode, whether Spark1 / `orchestrator-agent` was used, Codex role, and missing integration point. Current `spark1_orchestrated` requests fail closed with `STOP_AND_REPORT` because `run-batch-operator` does not yet implement a pre-dispatch Spark1 orchestrator handoff for worker assignment.
