...
- The local debug surface at `/nutrition/local-schema` now treats 16 columns as the target shape. Until the Thai correction slice is explicitly executed locally, the page should remain in an attention state for column count while still showing the current 14-column table correctly.
