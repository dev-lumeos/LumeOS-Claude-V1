...
| 13. Reporting / dossier | `system/reports/*`, `system/reports/batch-dossier.ts`, reports directories, `apps/web/src/app/governance/dossiers` | TESTED | Batch 006 adds a read-only batch dossier reporter; Governance UI V2 displays structured dossier timeline sections, output classification, checker summaries, git state, autonomy handoff, and raw JSON/stdout fallback through the allowlisted local API. |
| 14. Operator CLI | `system/workorders/cli/run-batch-operator.ts`, `batch-operator.ts`, `operator-doctor.ts`, runbook, tests, `apps/web/src/app/governance/*` | TESTED | Operator reached real DONE for Nutrition 001 and P1-004; Batch 008 adds Doctor mode; Operator Autonomy V2 adds stable `autonomy_handoff` output for STOP/FIX/approval states; Governance UI V2 exposes status, dry-run, doctor, continue, safe-cleanup continue, better doctor summaries, and copyable next-action commands through explicit command gates. |
...
- Codex Worker Bridge for constrained non-interactive `codex exec` prompt construction and `senior-coding-agent` worker invocation. Dispatcher integration is controlled-enabled for senior-coding-agent only and requires workorder opt-in plus source/scope/output/product-gate checks.
...
- Governance UI V2 for a lightweight workorder dependency board, structured dossier timeline, improved doctor/approval/runtime summaries, and copyable next-action command blocks.
- Project Profiles for the default LumeOS governance roots, raw local paths, forbidden artifacts, product-gate policy, and profile-aware command defaults.
