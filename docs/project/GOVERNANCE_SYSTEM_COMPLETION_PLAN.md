...
- Codex Worker Bridge for constrained non-interactive `codex exec` prompt construction and `senior-coding-agent` worker invocation. Dispatcher integration exists but auto-run remains disabled by default through config gates and workorder opt-in.
