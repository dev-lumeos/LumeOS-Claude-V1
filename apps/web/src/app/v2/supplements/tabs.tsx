...
export function SuppExtended() {
  ...
  return (
    <div className="v2-col-gap" style={{ gap: 14 }}>
      <div className="v2-insight v2-warn">
        <div className="v2-insight-mark" />
        <div style={{ flex: 1, minWidth: 0 }}>
          <div className="v2-insight-title">Extended protocol</div>
          <div className="v2-insight-body">
            Compounds outside the standard supplement range. Tracked for
            completeness — not a recommendation.
          </div>
        </div>
      </div>

      <Card title="Compounds" sub={`${EXTENDED_STACK.length} tracked`} attrappe={ATTRAPPE}
            actions={
              <button type="button" className="v2-btn v2-btn-sm" onClick={() => open('addCompound')}>
        ...
        <div className="v2-supp-tbl-wrap">
          <table className="v2-tbl">
            <tbody>
              {(EXTENDED_STACK as Array<Record<string, unknown>>).map((c, i) => (
                <tr key={String(c.name ?? i)} style={{ cursor: 'pointer' }}
                    onClick={() => open('logDose', c)}>
                  <td>
                    <div style={{ fontSize: 12.5, fontWeight: 500 }}>{String(c.name ?? '')}</div>
                    <div className="v2-muted" style={{ fontSize: 10.5 }}>{String(c.category ?? c.class ?? '')}</div>
                  </td>
                  <td className="v2-num" style={{ fontSize: 11.5 }}>{String(c.dose ?? '')}</td>
                  <td className="v2-muted" style={{ fontSize: 11 }}>{String(c.schedule ?? c.frequency ?? '')}</td>
      ...
      <Card title="Bloodwork" sub={`${EXTENDED_LABS.length} markers`} attrappe={ATTRAPPE}
            actions={
              <button type="button" className="v2-btn v2-btn-sm" onClick={() => open('addLab')}>
                <Icon name="plus" className="v2-ic v2-ic-sm" /> Add result
              </button>
            }>
        <div className="v2-supp-tbl-wrap">
          <table className="v2-tbl">
            <tbody>
              {(EXTENDED_LABS as Array<Record<string, unknown>>).map((l, i) => (
                <tr key={String(l.marker ?? l.name ?? i)}>
                  <td style={{ fontSize: 12.5 }}>{String(l.marker ?? l.name ?? '')}</td>
                  <td className="v2-num">{String(l.value ?? '')}</td>
                  <td className="v2-muted v2-num" style={{ fontSize: 11 }}>{String(l.ref ?? l.range ?? '')}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
export function SuppCompliance() {
  ...
  return (
    <>
      ...
      {view === 'calendar' ? (
        <Card title="Calendar · month" sub="adherence per day" attrappe={ATTRAPPE}>
          <div className="v2-supp-adherence">
            {Array.from({ length: 32 }).map((_, i) => (
              <div key={i} style={{
                aspectRatio: '1', borderRadius: 3,
                background: i % 7 === 6 ? 'var(--surface-2)'
                  : i % 5 === 0 ? 'var(--warn)' : 'var(--pos)',
                opacity: 0.75,
              }} />
            ))}
          </div>
          ...
          <Card title="Compliance heatmap" sub="last 90 days · all supplements" attrappe={ATTRAPPE}>
            <div className="v2-col-gap" style={{ gap: 4 }}>
              {STACK.slice(0, 6).map(s => (
                <div key={s.id} className="v2-supp-adherence-row">
                  <span style={{ fontSize: 11, color: 'var(--fg-muted)' }}>{s.name}</span>
                  <div className="v2-supp-adherence">
                    {Array.from({ length: 16 }).map((_, i) => (
                      <div key={i} style={{
                        height: 14, borderRadius: 2,
                        background: (i + s.streakDays) % 9 === 0 ? 'var(--warn)'
                          : (i + s.streakDays) % 13 === 0 ? 'var(--surface-2)' : 'var(--pos)',
                        opacity: 0.8,
                      }} />
                    ))}
                  </div>
