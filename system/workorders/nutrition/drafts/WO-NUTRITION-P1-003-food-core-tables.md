...
task: |
  <task>
    ...
    <implement>
      ...
      - nutrition.tag_definitions (Seed-INSERTs der 16 V1-Tags aus Decisions §5)
      - nutrition.food_tags (food_id + tag_code PK)

      Auto-Tag-Trigger:
      - Funktion nutrition.auto_tag_food(p_food_id UUID) — REDUZIERT auf die 16 V1-Tags
    ...
    <constraints>
      ...
      Auto-Tag-Trigger setzt ausschließlich diese 16 Tags.
    ...
    <on_error>
      ...
      Bei Tag-Set Drift (mehr/weniger als 16 V1-Tags geseedet): {"status": "FAIL"}.
    </on_error>
  </task>
...
acceptance_criteria:
  ...
  - "tag_definitions enthält genau die 16 V1-Tags aus NUTRITION_NEXT_SPEC_DECISIONS.md §5"
  - "Auto-Tag-Trigger trg_foods_auto_tag aktiv und auf die 16 V1-Tags beschränkt"
