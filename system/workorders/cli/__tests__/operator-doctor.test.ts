...
function baseStatus(overrides: Partial<OperatorStatus> = {}): OperatorStatus {
  return {
    ...
    documentationHandling: [{
      ...
      naReason: 'Test fixture has no documentation impact.',
      status: 'skipped_na',
      ssotSyncStatus: 'not_run',
      detail: 'Test fixture has no documentation impact.',
    }],
    relatedApprovals: [],
    approvalStops: [],
    cleanupSuggestions: [],
...
describe('operator doctor diagnosis', () => {
  ...
  it('treats an exact allowlisted batch as product-gate allowed in doctor output', () => {
    const result = diagnoseOperatorDoctor(baseStatus({
      ...
      projectProfile: {
        ...
        product_gate: {
          ...
          execution_batch_allowlist: ['system/workorders/nutrition/batches/BATCH-NUTRITION-P1-005-PREPARATION-DRAFT.md'],
        },
        forbidden_paths: [],
        forbidden_commands: [],
        required_checkers: [],
    ...
    assert.equal(result.product_gate.status, 'allowed')
    assert.match(result.product_gate.reason, /Scoped execution exception active/)
  })
