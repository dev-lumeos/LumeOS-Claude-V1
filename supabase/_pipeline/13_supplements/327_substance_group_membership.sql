-- C-327: Fachliche Substanzgruppen bleiben von Produktgruppen getrennt.
-- Der Kettenschritt 327a befuellt sie ausschliesslich aus Kimi-Regeldaten.
CREATE TABLE IF NOT EXISTS supplements.substance_group_memberships (
  substance_group_id TEXT NOT NULL CHECK (btrim(substance_group_id) <> ''),
  supplement_id UUID NOT NULL REFERENCES supplements.supplements(id) ON DELETE CASCADE,
  source TEXT NOT NULL CHECK (btrim(source) <> ''),
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  PRIMARY KEY (substance_group_id, supplement_id)
);

CREATE INDEX IF NOT EXISTS substance_group_memberships_supplement_idx
  ON supplements.substance_group_memberships(supplement_id);

ALTER TABLE supplements.substance_group_memberships ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS substance_group_memberships_select ON supplements.substance_group_memberships;
CREATE POLICY substance_group_memberships_select
  ON supplements.substance_group_memberships FOR SELECT TO authenticated USING (true);
GRANT SELECT ON supplements.substance_group_memberships TO authenticated;
GRANT ALL ON supplements.substance_group_memberships TO service_role;
DROP TRIGGER IF EXISTS substance_group_memberships_touch_updated_at ON supplements.substance_group_memberships;
CREATE TRIGGER substance_group_memberships_touch_updated_at
  BEFORE UPDATE ON supplements.substance_group_memberships
...
BEGIN
  ...
  SELECT COALESCE(array_agg(DISTINCT membership.substance_group_id), '{}') INTO v_stack_groups
  FROM supplements.user_stacks us
  JOIN supplements.stack_items si ON si.stack_id = us.id
  JOIN supplements.supplements s ON s.id = si.supplement_id
  JOIN supplements.substance_group_memberships membership ON membership.supplement_id = s.id
  WHERE us.user_id = p_user_id
    AND us.is_active
    AND si.is_active;
  ...
  LOOP
    ...
    AND EXISTS (
      SELECT 1
      FROM unnest(v_rule.substance_group_ids) AS required_group(substance_group_id)
      WHERE NOT EXISTS (
        SELECT 1
        FROM supplements.substance_group_memberships membership
        WHERE membership.substance_group_id = required_group.substance_group_id
      )
    ) THEN
    ...
    LOOP
      ...
      ELSIF v_module = 'supplements' AND v_field = 'stack' THEN
        IF v_op = 'contains_any_substance' THEN
          v_ok := v_ok AND (v_stack_substances && v_rule.substance_ids);
        ELSIF v_op = 'contains_any_group' THEN
          IF COALESCE(array_length(v_rule.substance_ids, 1), 0) > 0 THEN
            v_ok := v_ok AND (v_stack_substances && v_rule.substance_ids);
          ELSE
            v_ok := v_ok AND (v_stack_groups && v_rule.substance_group_ids);
          END IF;
        ELSIF v_op = 'count_group_gte' THEN
          IF COALESCE(array_length(v_rule.substance_ids, 1), 0) > 0 THEN
            SELECT count(*) INTO v_count
            FROM unnest(v_stack_substances) AS stack_substance(slug)
            WHERE stack_substance.slug = ANY(v_rule.substance_ids);
          ELSE
            SELECT count(DISTINCT s.slug) INTO v_count
            FROM unnest(v_stack_substances) AS stack_substance(slug)
            JOIN supplements.supplements s ON s.slug = stack_substance.slug
            JOIN supplements.substance_group_memberships membership ON membership.supplement_id = s.id
            WHERE membership.substance_group_id = ANY(v_rule.substance_group_ids);
          END IF;
          v_ok := v_ok AND v_count >= COALESCE((v_value->>'count')::integer, 1);
        ELSE
          v_ok := false;
