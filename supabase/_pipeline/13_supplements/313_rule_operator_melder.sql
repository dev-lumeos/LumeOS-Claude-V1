-- C-313b: Nicht implementierte Regeloperatoren muessen als eigener
-- Auswertungszustand sichtbar bleiben. Diese Funktion ist zugleich der
-- maschinenlesbare Vertrag fuer den Gate-Waechter.
...
AS $$
  SELECT CASE
    ...
    WHEN p_operator IN ('contains_cyp_inducer', 'contains_cyp_inhibitor', 'contains_cyp_substrate',
      ...
      AND p_module = 'medical' AND p_field = 'medications' THEN true
    WHEN p_operator = 'missing_analyte_within_months'
      AND p_module = 'medical' AND p_field = 'labs' THEN true
...
COMMENT ON FUNCTION supplements.rule_operator_supported(TEXT, TEXT, TEXT, TEXT) IS
  'C-313b: Zentraler Operatorvertrag fuer rule_assessment und den Gate-Waechter.';
COMMENT ON FUNCTION supplements.rule_assessment(UUID, DATE) IS
  'C-133/C-313b: Nutzerbezogene Regelauswertung mit fulfilled, not_fulfilled, missing_input oder unsupported_operator.';
