...
BEGIN
  ...
  IF v_rule_members IS DISTINCT FROM 8 THEN
    RAISE EXCEPTION 'C-393: wr_chelation_timing has % members instead of 8', v_rule_members;
  END IF;

  IF v_memberships IS DISTINCT FROM v_rule_members THEN
    RAISE EXCEPTION 'C-393: chelating-mineral memberships % instead of %', v_memberships, v_rule_members;
  END IF;
END;
$$;
