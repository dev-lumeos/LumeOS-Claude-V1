...
await p.screenshot({ path: 'backup/g336-vorher-planner.png', fullPage: true })

// ── Diary: das inline-Formular ──────────────────────────────────────
await p.goto(`${B}/v2/nutrition?tab=diary`, { waitUntil: 'networkidle' })
await p.waitForTimeout(2500)
await p.screenshot({ path: 'backup/g336-vorher-diary-zu.png', fullPage: true })
...
if (await knopf.count()) {
  ...
  await p.screenshot({ path: 'backup/g336-vorher-diary-offen.png', fullPage: true })
}
